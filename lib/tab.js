...
function abstractTable(){
    ...
    this.getColNum = function(){
        if(this.merger_header!=[]){
            this.colNum = this.merger_header.length;
            return   this.colNum;
        }else if(this.header != [] ){
...
function table(options){
    ...
    this.showHeader = function(){
        ...
        if(this.merger_header.length>0){
            var  $thead = $("<thead></thead>"),
                $tr = $("<tr>"),
                $th;
            var $tr2=$("<tr>"),
                $th2;
            for(var i=0;i<this.merger_header.length;i++){
                if(this.merger_header[i] instanceof Array!=true){
                    if(this.merger_header[i+1] instanceof Array==true){
                        $th = $("<th colspan='2'>").html(this.merger_header[i]);
                    }else{
                        $th = $("<th rowspan='2'>").html(this.merger_header[i]);
                    }
                    $th.appendTo($tr);
                    $tr.appendTo($thead);
                }else{
                    if(this.merger_header[i] instanceof Array==true){
                        for(var t=0;t<this.merger_header[i].length;t++){
                            $th2=$("<th>").html(this.merger_header[i][t]);
                            $th2.appendTo($tr2);
                        }
                        $tr2.appendTo($thead);
                    }
    ...
    this.init = function(begin,end){
        if(options.headers==undefined){
            this.merger_header = options.merge_headers;
            this.colNum = this.merger_header.length;
        }else{
